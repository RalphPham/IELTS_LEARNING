...
import { SEQUENCING_INTRO, SEQUENCING_RULES } from '@/data/grammar/sequencing'
...
const seedCount = computed(() => questionsForTense('sequencing').length)
const userCount = computed(
  () => store.userQuestions.filter((q) => q.tenseId === 'sequencing').length,
)
const total = computed(() => seedCount.value + userCount.value)
const progress = computed(() => store.getForTense('sequencing'))
</script>
  ...
  <div class="mx-auto max-w-3xl px-4 lg:px-8 py-6">
    ...
    <header class="mb-5">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-2xl font-bold text-slate-900">Sự phối thì</h1>
          <p class="text-sm text-slate-500">Tense Coordination — chia động từ theo liên từ thời gian</p>
        </div>
      </div>
    ...
    <!-- Rules -->
    <section
      v-for="rule in SEQUENCING_RULES"
    ...
    >
      <div class="flex items-baseline gap-2 mb-3 flex-wrap">
        <h2 class="text-lg font-black text-indigo-700 uppercase tracking-wide">{{ rule.conjunction }}</h2>
    ...
    <section class="rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-600 text-white shadow-lg p-5">
      ...
      <div class="flex flex-wrap gap-2 mt-4">
        <RouterLink
          to="/grammar/sequencing/practice"
        ...
        <RouterLink
          ...
          to="/grammar/sequencing/practice?wrong=1"
